...
const AddressSection = () => {
    ...
    return (
        <section className={section}>
            ...
            <div 
                className={returnTrip.isReturnTrip ? button + ' text-red-500 hover:text-red-400' : button + ' ' } 
                onClick={()=>{
                    setIsReturnTrip(!returnTrip.isReturnTrip)
                    setIsReturn(!validation.isReturn)
                }}
            >
                {!validation.isReturn && <CgArrowsExchangeAlt className="text-3xl"/>}
                {validation.isReturn && <CgClose className="text-3xl"/>}
            </div>
